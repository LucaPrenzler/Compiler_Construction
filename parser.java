...
public class parser {
    ...
    public void S() {
        ...
        if (tokens.getCode().matches("halt") || tokens.getCode().matches("input") || tokens.getCode().matches("output")
                || tokens.getCode().matches("if") || tokens.getCode().matches("while")
                || tokens.getCode().matches("for") && tokens.next != null) {
            A(node);
            match("$", node);
    ...
    public void ASS(ParserNode node) {
        ...
        } else if (tokens.getType().matches("Number") || tokens.getCode().matches("sub")
                || tokens.getCode().matches("add") || tokens.getCode().matches("mult") && tokens.next != null) {
            N(node);
        } else if (tokens.getCode().matches("$")) {
            match("$", node);
        } else {
    ...
    public void N(ParserNode p) {
        ...
        } else if (tokens.getCode().matches("sub") || tokens.getCode().matches("add")
                || tokens.getCode().matches("mult") && tokens.next != null) {
            CA(node);
    ...
    public void BB(ParserNode node) {
        ...
        if (tokens.getCode().equalsIgnoreCase("(") || tokens.getCode().matches("eq") || tokens.getCode().matches("not")
                || tokens.getCode().matches("and") || tokens.getCode().matches("or") && tokens.next != null) {
            B(node);
            match(",", node);
            B(node);
            match(")", node);
